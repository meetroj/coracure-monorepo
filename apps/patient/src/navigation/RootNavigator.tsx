...
import InstantConsultRequestScreen from '../screens/InstantConsultRequestScreen';
...
export const RootNavigator = () => {
  ...
  const navigationValue = useMemo(() => {
    return {
      /**
       * Matches React Navigation: navigating to a screen already in the stack
       * returns to it rather than pushing a duplicate. Without this every tab
       * press stacked another entry, so Back retraced each one instead of
       * returning to the screen the user actually came from.
       */
      navigate: (screen: any, params?: any) => {
        setStack((prev) => {
          const idx = prev.findIndex((entry) => entry.name === screen);
          if (idx === -1) {
            return [...prev, { key: `${screen}-${Date.now()}`, name: screen, params }];
          }
          const next = prev.slice(0, idx + 1);
          next[idx] = { ...next[idx], params: params ?? next[idx].params };
          return next;
        });
      },
  ...
  const renderScreenContent = () => {
    switch (current.name) {
      ...
      case 'SelectSlot':
        return <SelectSlotScreen />;
      case 'InstantConsultRequest':
        return <InstantConsultRequestScreen />;
      case 'CaregiverGuide':
  ...
  const ALL_SCREENS: ScreenItem[] = [
    ...
    { name: 'ChooseService', label: '6c1. Choose a Service' },
    { name: 'FindDoctor', label: '6c2. Find a Doctor' },
    { name: 'DoctorProfile', label: '6c3. Doctor Profile' },
    { name: 'SelectSlot', label: '6c4. Select a Slot' },
    { name: 'InstantConsultRequest', label: '6c5. Instant Consult Request' },
    ...
    { name: 'AppointmentDetail', label: '7b. Appointment Detail', params: { consultationId: 'cons-001' } },
...
const s = StyleSheet.create({
  container: {
    ...
    height: '100%',
    /**
     * White rather than the page tint: this view owns the safe-area padding,
     * so its colour is what shows in the status-bar and gesture-bar strips.
     * White matches the tab bar and header chrome that sit against them.
     */
